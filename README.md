<img src="public/icons/icon-72.png" alt="생활도구" width="32" height="32" align="left" style="margin-right: 8px;">

# Productivity Tools

React + TypeScript + Vite 기반의 유틸리티 앱 대시보드

## 아키텍처: 프론트엔드 전용 (백엔드 없음)

이 프로젝트는 **100% 클라이언트 사이드 애플리케이션**입니다.

| 특징 | 설명 |
|:-----|:-----|
| 백엔드 없음 | 서버, API, 데이터베이스가 없습니다 |
| 오프라인 지원 | PWA로 인터넷 없이도 모든 기능 사용 가능 |
| 로컬 저장소 | 모든 데이터는 브라우저의 localStorage에 저장 |
| 개인정보 보호 | 사용자 데이터가 서버로 전송되지 않음 |

> 이 아키텍처는 개인정보 보호, 성능, 오프라인 안정성을 위해 의도적으로 선택되었습니다.

## 앱 목록

| 앱 | 설명 | 경로 |
|:---|:-----|:-----|
| 🎵 메트로놈 | Metronome | `/metronome` |
| 📱 QR 코드 생성기 | QR Code Generator | `/qr` |

## 시작하기

```bash
# 설치
npm install

# 개발 서버 실행
npm run dev

# 프로덕션 빌드
npm run build

# 전체 검증 (타입체크 + 린트 + 테스트)
npm run validate
```

## 기술 스택

| 분류 | 기술 |
|:-----|:-----|
| 프레임워크 | React 18 |
| 라우팅 | React Router 7 |
| 언어 | TypeScript 5 |
| 빌드 | Vite 5 |
| 테스트 | Vitest + React Testing Library |
| 배포 | Cloudflare Pages |

## UI 최적화

- UI는 **14인치 MacBook** 기준으로 1차 최적화되어 있습니다
- 다른 화면 크기에서도 사용 가능하지만, 14인치 MacBook에서 가장 최적의 경험을 제공합니다

## 라이선스

### 소스코드 (MIT License)

이 프로젝트의 **소스코드**는 MIT 라이선스로 공개됩니다. 저작권 고지를 포함하여 자유롭게 사용, 수정, 배포할 수 있습니다.

자세한 내용은 [LICENSE](./LICENSE) 파일을 참조하세요.

### 지적재산권 및 상표 (Intellectual Property & Trademarks)

**다음 항목들은 MIT 라이선스의 적용을 받지 않으며, 무단 사용이 금지됩니다:**

| 항목 | 설명 |
|:-----|:-----|
| 브랜드명 | "SoundBlueMusic", "Sound Blue" 및 모든 변형 |
| 로고 | 모든 로고, 아이콘 및 브랜드 이미지 |
| 이미지 | 프로젝트에 포함된 모든 이미지 및 그래픽 |
| 음악/오디오 | 프로젝트에 포함된 모든 음악, 음향 효과 및 오디오 파일 |
| 브랜드 자산 | 기타 모든 브랜드 관련 자료 |

**주의사항:**
- 위 항목들의 무단 사용, 복제, 수정, 배포는 금지됩니다
- 이 프로젝트를 포크하거나 소스코드를 사용할 경우, 보호되는 모든 자료(로고, 이미지, 오디오, 브랜드명)를 제거하고 본인의 것으로 교체해야 합니다

**© 2025 SoundBlueMusic. All Rights Reserved for Intellectual Property & Trademarks.**

---

## Inspiration

Looking for ideas? Visit the [Claude Artifacts Gallery](https://claude.ai/artifacts) and check out the **Inspiration** tab to discover creative artifacts built by others with Claude.

---

Built with [Claude Code](https://docs.anthropic.com/en/docs/claude-code)
