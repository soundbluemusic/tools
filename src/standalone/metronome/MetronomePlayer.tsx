...
const BPM_RANGE = { MIN: 40, MAX: 240 } as const;
const VOLUME_RANGE = { MIN: 0, MAX: 100 } as const;
const FREQUENCIES = { ACCENT: 2000, REGULAR: 800 } as const;
const TIMING = {
  SCHEDULER_INTERVAL_MS: 25,
  LOOK_AHEAD_SECONDS: 0.1,
  CLICK_DURATION_SECONDS: 0.08,
} as const;
const PENDULUM = { MAX_ANGLE: 30, SWING_RANGE: 60 } as const;
...
const MetronomePlayer = memo<MetronomePlayerProps>(function MetronomePlayer({
  ...
  useEffect(() => {
    if (isPlaying && audioContextRef.current) {
      const animate = () => {
        if (!audioContextRef.current || startAudioTimeRef.current === 0) {
