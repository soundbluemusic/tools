<script lang="ts">
  ...
  import { QRGenerator } from '$lib/apps/qr';
</script>

<svelte:head>
  <title>{$t.qr.seo.title}</title>
  <meta name="description" content={$t.qr.seo.description} />
  <meta name="keywords" content={$t.qr.seo.keywords} />
  <link rel="canonical" href="https://tools.soundblue.net/qr" />
</svelte:head>
...
<div class="page-layout">
  <nav class="breadcrumb" aria-label="Breadcrumb">
    <ol class="breadcrumb-list">
      ...
      <li class="breadcrumb-item"><a href="/" class="breadcrumb-link">{$language === 'ko' ? '유틸리티 도구' : 'Utility Tools'}</a></li>
      <li class="breadcrumb-separator" aria-hidden="true">/</li>
      <li class="breadcrumb-item breadcrumb-current" aria-current="page">{$t.qr.title}</li>
    </ol>
  </nav>

  <header class="page-header">
    <h1 class="page-title">{$t.qr.title}</h1>
    <p class="page-description">{$t.qr.subtitle}</p>
  </header>
