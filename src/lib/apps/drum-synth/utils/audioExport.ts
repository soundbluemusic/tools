/**
 * Audio Export Utilities for Drum Synth
 */
...
function renderKick(ctx: OfflineAudioContext, kickParams: KickParams, masterVolume: number): void {
  const now = 0;
...
function renderSnare(ctx: OfflineAudioContext, snareParams: SnareParams, masterVolume: number): void {
  const now = 0;
  const volume = masterVolume / 100;
...
function renderHihat(ctx: OfflineAudioContext, hihatParams: HihatParams, masterVolume: number): void {
  const now = 0;
  const volume = masterVolume / 100;
...
function renderClap(ctx: OfflineAudioContext, clapParams: ClapParams, masterVolume: number): void {
  const now = 0;
  const volume = masterVolume / 100;
  ...
  for (let i = 0; i < crackBuffer.length; i++) {
    const t = i / ctx.sampleRate;
    crackData[i] = (Math.random() * 2 - 1) * Math.exp(-t / 0.003);
  }
...
function renderTom(ctx: OfflineAudioContext, tomParams: TomParams, masterVolume: number): void {
  const now = 0;
  const volume = masterVolume / 100;
  const osc = ctx.createOscillator();
  const gainNode = ctx.createGain();
...
function renderRim(ctx: OfflineAudioContext, rimParams: RimParams, masterVolume: number): void {
  const now = 0;
  const volume = masterVolume / 100;
...
function getDrumDuration(drumType: DrumType, params: AllDrumParams): number {
  switch (drumType) {
    case 'kick': return params.kick.ampDecay + 0.2;
    case 'snare': return Math.max(params.snare.toneDecay, params.snare.noiseDecay) + 0.2;
    case 'hihat': return params.hihat.decay + (params.hihat.openness / 100) * 0.3 + 0.2;
    case 'clap': return params.clap.decay + (params.clap.reverb / 100) * 0.5 + 0.3;
    case 'tom': return params.tom.decay + 0.2;
    case 'rim': return params.rim.decay * 1.5 + 0.2;
    default: return 1;
  }
}

export async function renderDrumToBuffer(drumType: DrumType, params: AllDrumParams): Promise<AudioBuffer> {
  ...
  switch (drumType) {
    case 'kick': renderKick(ctx, params.kick, masterVolume); break;
    case 'snare': renderSnare(ctx, params.snare, masterVolume); break;
    case 'hihat': renderHihat(ctx, params.hihat, masterVolume); break;
    case 'clap': renderClap(ctx, params.clap, masterVolume); break;
    case 'tom': renderTom(ctx, params.tom, masterVolume); break;
    case 'rim': renderRim(ctx, params.rim, masterVolume); break;
  }
...
export async function audioBufferToMp3(buffer: AudioBuffer): Promise<Blob> {
  ...
  for (const type of mimeTypes) {
    if (MediaRecorder.isTypeSupported(type)) { supportedType = type; break; }
  }
  if (!supportedType) return audioBufferToWav(buffer);
  ...
  return new Promise((resolve, reject) => {
    ...
    const mediaRecorder = new MediaRecorder(destination.stream, { mimeType: supportedType });
    const chunks: Blob[] = [];
    mediaRecorder.ondataavailable = (e) => { if (e.data.size > 0) chunks.push(e.data); };
    mediaRecorder.onstop = () => { audioContext.close(); resolve(new Blob(chunks, { type: supportedType })); };
    mediaRecorder.onerror = (e) => { audioContext.close(); reject(e); };
    mediaRecorder.start();
    source.start(0);
    setTimeout(() => { source.stop(); mediaRecorder.stop(); }, (buffer.duration + 0.1) * 1000);
  });
}
...
export async function exportDrum(drumType: DrumType, params: AllDrumParams, format: ExportFormat = 'wav'): Promise<void> {
  ...
  downloadBlob(blob, `${drumType}_${Date.now()}.${extension}`);
}

export async function exportAllDrums(params: AllDrumParams, format: ExportFormat = 'wav'): Promise<void> {
