<script lang="ts">
  ...
  let beatsPerMeasure = $state(DEFAULTS.BEATS_PER_MEASURE);
  let beatUnit = $state(DEFAULTS.BEAT_UNIT);
  ...
  // Current values refs (for use in callbacks)
  let currentBpm = bpm;
  ...
  // Get accent pattern - only first beat is accented
  ...
  function scheduleNotes() {
    ...
    while (nextNoteTime < now + TIMING.LOOK_AHEAD_SECONDS) {
      ...
      schedulerBeat = (schedulerBeat + 1) % currentBeatsPerMeasure;
    }
  }

  // Animation loop
  function animate() {
    if (!audioContext || startAudioTime === 0) {
    ...
    const currentBeatIndex = Math.floor(totalBeats) % currentBeatsPerMeasure;
    const currentMeasure = Math.floor(totalBeats / currentBeatsPerMeasure) + 1;
    ...
    if (!isPlaying) {
      if (audioContext) {
        ...
        if (elapsedTime === 0 && countdownTime === 0) {
          ...
          // Resume from pause
          const elapsedSeconds = currentElapsedTime / 1000;
          ...
          schedulerBeat = (currentBeatNumber + 1) % currentBeatsPerMeasure;
          nextNoteTime = startAudioTime + (currentBeatNumber + 1) * secondsPerBeat;
        }
      }
  ...
  const currentCountdown = $derived(countdownTime > 0 ? Math.max(0, countdownTime - countdownElapsed) : 0);
...
<div class="metronome">
  ...
  <div class="metronome-controls">
    ...
    <div class="metronome-control-group">
      <span class="metronome-label">{$t.metronome.timeSignature}</span>
      <input
        type="number"
        min="1"
        max="12"
    ...
    <!-- Timer -->
    <div class="metronome-control-group">
      <span class="metronome-label">{$t.metronome.timer}</span>
      <input
        type="number"
        min="0"
        max="99"
      ...
      />
      <span class="metronome-unit">{$t.metronome.minutes}</span>
      <input
        type="number"
        min="0"
        max="59"
        ...
        disabled={isPlaying || elapsedTime > 0}
      />
      <span class="metronome-unit">{$t.metronome.seconds}</span>
      <span class="metronome-separator"></span>
      <div class="metronome-countdown">
        <span class="metronome-countdown-label">{$t.metronome.countdown}</span>
  ...
  <div class="metronome-main">
    ...
    <div class="metronome-display">
      <div class="metronome-display-label">{$t.metronome.bpm}</div>
    ...
    <!-- Volume display -->
    <div class="metronome-display">
      <div class="metronome-display-label">{$t.metronome.volume}</div>
  ...
  <div class="metronome-sliders">
    <div class="metronome-slider-group">
      ...
      <div class="metronome-slider-labels">
        <span>{$t.metronome.slow}</span>
        <span>{$t.metronome.fast}</span>
      </div>
    </div>
    ...
    <div class="metronome-slider-group">
      ...
      />
      <div class="metronome-slider-labels">
        <span>{$t.metronome.quiet}</span>
        <span>{$t.metronome.loud}</span>
      </div>
    </div>
  </div>
    ...
    {#each Array(beatsPerMeasure) as _, i}
      <div class="metronome-beat">
        {#if beatUnit === 2}
          <!-- Half note -->
          <svg
            width={isPlaying && i === beat ? (i === 0 ? 28 : 24) : 16}
            height={(isPlaying && i === beat ? (i === 0 ? 28 : 24) : 16) * 1.4}
            viewBox="0 0 24 34"
            fill="none"
          ...
          >
            <ellipse
              ...
              ry="5"
              fill="var(--color-bg-tertiary)"
              stroke={isPlaying && i === beat ? (i === 0 ? 'var(--color-error)' : 'var(--color-text-primary)') : 'var(--color-text-tertiary)'}
              stroke-width="2"
            />
            <line x1="19" y1="24" x2="19" y2="4" stroke={isPlaying && i === beat ? (i === 0 ? 'var(--color-error)' : 'var(--color-text-primary)') : 'var(--color-text-tertiary)'} stroke-width="2" />
          </svg>
        {:else if beatUnit === 4}
          <!-- Quarter note -->
          <svg
            ...
            class="metronome-note"
          >
            <ellipse cx="12" cy="24" rx="7" ry="5" fill={isPlaying && i === beat ? (i === 0 ? 'var(--color-error)' : 'var(--color-text-primary)') : 'var(--color-text-tertiary)'} />
            <line x1="19" y1="24" x2="19" y2="4" stroke={isPlaying && i === beat ? (i === 0 ? 'var(--color-error)' : 'var(--color-text-primary)') : 'var(--color-text-tertiary)'} stroke-width="2" />
          </svg>
        {:else if beatUnit === 8}
          <!-- Eighth note -->
          ...
          >
            <ellipse cx="12" cy="24" rx="7" ry="5" fill={isPlaying && i === beat ? (i === 0 ? 'var(--color-error)' : 'var(--color-text-primary)') : 'var(--color-text-tertiary)'} />
            <line x1="19" y1="24" x2="19" y2="4" stroke={isPlaying && i === beat ? (i === 0 ? 'var(--color-error)' : 'var(--color-text-primary)') : 'var(--color-text-tertiary)'} stroke-width="2" />
            <path d="M19 4 L19 10 L24 8 L24 2 Z" fill={isPlaying && i === beat ? (i === 0 ? 'var(--color-error)' : 'var(--color-text-primary)') : 'var(--color-text-tertiary)'} />
          </svg>
        {:else if beatUnit === 16}
          <!-- Sixteenth note -->
          <svg
          ...
          >
            ...
            <line x1="19" y1="24" x2="19" y2="4" stroke={isPlaying && i === beat ? (i === 0 ? 'var(--color-error)' : 'var(--color-text-primary)') : 'var(--color-text-tertiary)'} stroke-width="2" />
            <path d="M19 4 L19 10 L24 8 L24 2 Z" fill={isPlaying && i === beat ? (i === 0 ? 'var(--color-error)' : 'var(--color-text-primary)') : 'var(--color-text-tertiary)'} />
            <path d="M19 8 L19 14 L24 12 L24 6 Z" fill={isPlaying && i === beat ? (i === 0 ? 'var(--color-error)' : 'var(--color-text-primary)') : 'var(--color-text-tertiary)'} />
          </svg>
        {/if}
      </div>
    {/each}
  ...
  <div class="metronome-info">
    <div class="metronome-info-item">
      <div class="metronome-info-label">{$t.metronome.measure}</div>
      <div class="metronome-info-value">{measureCount}</div>
    </div>
    <div class="metronome-info-item">
      <div class="metronome-info-label">{$t.metronome.elapsed}</div>
      <div class="metronome-info-value metronome-info-value--mono">
        {formatTime(elapsedTime)}
      </div>
      <div class="metronome-info-precision">{$t.metronome.precision}</div>
    </div>
  </div>

  <!-- Action buttons -->
  <div class="metronome-actions">
    <button onclick={handleStart} class="metronome-btn metronome-btn--primary">
      {#if isPlaying}
  ...
  <div class="metronome-sync">
    <div class="metronome-sync-title">{$t.metronome.perfectSync}</div>
    <div class="metronome-sync-desc">{$t.metronome.syncDescription}</div>
  </div>
</div>
