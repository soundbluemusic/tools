...
export const PWAPrompt = memo(function PWAPrompt() {
  ...
  if (needsUpdate) {
    return (
      <div className="pwa-banner pwa-banner--update" role="alert">
        ...
        <button className="pwa-banner__button" onClick={update}>
          {pwa.updateButton}
        </button>
      </div>
    );
  ...
  if (isInstallable && !isDismissed && !isInstalled) {
    return (
      <div className="pwa-banner pwa-banner--install" role="banner">
        <div className="pwa-banner__content">
          <div className="pwa-banner__icon">
            <svg viewBox="0 0 24 24" fill="currentColor" width="24" height="24">
        ...
        <div className="pwa-banner__actions">
          <button
            className="pwa-banner__button pwa-banner__button--secondary"
            onClick={handleDismiss}
          >
            {pwa.dismissButton}
          </button>
          <button className="pwa-banner__button" onClick={handleInstall}>
            {pwa.installButton}
          </button>
        </div>
      </div>
