/**
 * Other Tools Category Page - Vanilla TypeScript
 */

import { Component, html } from '../../core/Component';
import { APPS, type AppConfig } from '../config';

const OTHER_APP_PATHS = ['/qr'];

const TRANSLATIONS = {
  ko: {
    title: '기타 도구',
    description: '생활에 유용한 유틸리티 도구들',
    breadcrumbHome: '홈',
    noTools: '도구가 없습니다.',
  },
  en: {
    title: 'Other Tools',
    description: 'Useful utility tools for everyday use',
    breadcrumbHome: 'Home',
...
export class OtherToolsPage extends Component<
  OtherToolsPageProps,
  OtherToolsPageState
> {
  protected getInitialState(): OtherToolsPageState {
  ...
  private get otherApps(): AppConfig[] {
    return APPS.filter((app) => OTHER_APP_PATHS.includes(app.url));
  }

  protected render(): string {
    const { language } = this.state;
    const apps = this.otherApps;
