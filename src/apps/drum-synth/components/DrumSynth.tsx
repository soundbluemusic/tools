  ...
  return (
    <div class="synth-param">
      <div class="synth-param-header">
        <span class="synth-param-label">{props.label}</span>
        <span class="synth-param-value">
          {displayValue()}
          {props.unit || ''}
        </span>
      </div>
 ...
 */
export const DrumSynth: Component<DrumSynthProps> = (props) => {
  // Use provided translations (standalone) or i18n context (main site)
  const contextTranslations = useTranslations();
  const drumSynth = () => props.translations ?? contextTranslations().drumSynth;
  ...
  const params = () => props.params ?? internalParams();
  const setParams = (
    ...
    if (props.onParamsChange) {
      ...
      props.onParamsChange(resolvedParams);
    } else {
      // Uncontrolled mode - update internal state
      setInternalParams(newParams);
    }
  };
  const [isPlaying, setIsPlaying] = createSignal<DrumType | null>(null);
   ...
   */
  const makeDistortionCurve = (amount: number): Float32Array<ArrayBuffer> => {
   ...
   * Play Kick sound
   */
  const playKick = (kickParams: KickParams, masterVolume: number) => {
   ...
   * Play Snare sound
   */
  const playSnare = (snareParams: SnareParams, masterVolume: number) => {
    const ctx = getAudioContext();
    if (!ctx) return;
   ...
   * Play Hi-Hat sound
   */
  const playHihat = (hihatParams: HihatParams, masterVolume: number) => {
    const ctx = getAudioContext();
    if (!ctx) return;
   ...
   * Play Clap sound
   */
  const playClap = (clapParams: ClapParams, masterVolume: number) => {
    const ctx = getAudioContext();
    if (!ctx) return;
   ...
   * Play Tom sound
   */
  const playTom = (tomParams: TomParams, masterVolume: number) => {
    const ctx = getAudioContext();
    if (!ctx) return;
   ...
   * Play Rim sound
   */
  const playRim = (rimParams: RimParams, masterVolume: number) => {
    const ctx = getAudioContext();
    if (!ctx) return;
   ...
   */
  const playDrum = (drumType: DrumType) => {
    const ctx = getAudioContext();
  ...
  return (
    <div class="drum-synth">
      ...
      <div class="synth-content">
        ...
        <div class="synth-params">
          <div class="synth-params-header">
            <h3 class="synth-params-title">
              {getDrumLabel(selectedDrum())} {drumSynth().parameters}
            </h3>
            <button
              class="synth-reset-btn"
              onClick={resetParams}
      ...
      <div class="synth-master">
        <h3 class="synth-section-title">{drumSynth().master}</h3>
        <ParamSlider
      ...
      <div class="synth-presets">
        <span class="synth-presets-label">{drumSynth().presets}</span>
        <div class="synth-presets-buttons">
          <For each={Object.keys(SYNTH_PRESETS)}>
            {(preset) => {
      ...
      <div class="synth-export">
        <span class="synth-export-label">{drumSynth().export}</span>
        ...
        <Show when={isExporting()}>
          <div class="synth-export-status">{drumSynth().exporting}</div>
        </Show>
      </div>
      ...
      <Show when={statusMessage()}>
        <div
          class={cn('synth-status', `synth-status--${statusMessage()?.type}`)}
        >
          {statusMessage()?.text}
        </div>
      </Show>
