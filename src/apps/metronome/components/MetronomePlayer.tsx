...
const MetronomePlayer = memo(function MetronomePlayer() {
  ...
  const [beatsPerMeasure, setBeatsPerMeasure] = useState(DEFAULTS.BEATS_PER_MEASURE);
  const [beatUnit, setBeatUnit] = useState(DEFAULTS.BEAT_UNIT);
  ...
  const bpmRef = useRef(DEFAULTS.BPM);
  const volumeRef = useRef(DEFAULTS.VOLUME);
  const beatsPerMeasureRef = useRef(DEFAULTS.BEATS_PER_MEASURE);
