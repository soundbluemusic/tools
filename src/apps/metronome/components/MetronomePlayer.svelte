<script lang="ts">
  ...
  function scheduleNote(time: number, isAccent: boolean) {
    ...
    osc.frequency.value = isAccent ? FREQUENCIES.ACCENT : FREQUENCIES.NORMAL;
    osc.type = 'sine';

    gain.gain.setValueAtTime(volume / 100, time);
    gain.gain.exponentialRampToValueAtTime(0.001, time + TIMING.NOTE_DURATION);

    osc.start(time);
    osc.stop(time + TIMING.NOTE_DURATION);
  }

  function scheduler() {
    if (!audioContext) return;

    while (nextNoteTime < audioContext.currentTime + TIMING.SCHEDULE_AHEAD) {
